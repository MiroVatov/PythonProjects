...
class DefenderTests(unittest.TestCase):
    def setUp(self) -> None:
        self.player = Defender(name="Miro", height=179, weight=81, salary=100000, contract_expiry_date="10-10-2025",
                                 stamina=100, transfer_fee=10_000_000, positioning=10, tackling=10, heading=10, pace=10)
    ...
    def test__init__method_successful(self):
        ...
        self.assertEqual(10, self.player.positioning)
        self.assertEqual(10, self.player.tackling)
        self.assertEqual(10, self.player.heading)
    ...
    def test_name_raises_value_error(self):
        with self.assertRaises(ValueError) as err_msg:
            self.player = Defender(name="", height=179, weight=81, salary=100000, contract_expiry_date="10-10-2025",
                                   stamina=100, transfer_fee=10_000_000, positioning=10, tackling=10, heading=10,
                                   pace=10)
        self.assertEqual("Player name cannot be an empty string.", str(err_msg.exception))

    def test_height_raises_value_error(self):
        with self.assertRaises(ValueError) as err_msg:
            self.player = Defender(name="Miro", height=149, weight=81, salary=100000, contract_expiry_date="10-10-2025",
                                   stamina=100, transfer_fee=10_000_000, positioning=10, tackling=10, heading=10,
                                   pace=10)
    ...
    def test_salary_raises_value_error(self):
        with self.assertRaises(ValueError) as err_msg:
            self.player = Defender(name="Miro", height=179, weight=81, salary=1000, contract_expiry_date="10-10-2025",
                                   stamina=100, transfer_fee=10_000_000, positioning=10, tackling=10, heading=10,
                                   pace=10)
