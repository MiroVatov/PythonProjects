    ...
    def calculate_team_strength(self):
        ...
        for player in self.players:
            if isinstance(player, Goalkeeper):
                player_qualities = player.stamina + player.handling + player.jumping + player.reflexes
                team_strength += player_qualities
            elif isinstance(player, Defender):
                player_qualities = player.stamina + player.positioning + player.heading + player.pace + player.tackling
                team_strength += player_qualities
            elif isinstance(player, Midfielder):
                player_qualities = player.stamina + player.pace + player.agression + player.creativity + player.shooting + player.creativity
                team_strength += player_qualities
            elif isinstance(player, Forward):
                player_qualities = player.stamina + player.heading + player.goalscoring + player.speed + player.technique
                team_strength += player_qualities
            if self.coach is not None:
                team_strength += self.coach.calculate_coach_strength()
